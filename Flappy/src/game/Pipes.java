...
public class Pipes implements Renderable, Updatable {
	...
	private int getRandomY() {
		return rand.nextInt((int)(Menu.HEIGHT * 0.4f)) + (Menu.HEIGHT / 10);
	}
	...
	public void render(Graphics2D g, float interpolation) {
		...
		g.fillRect((int) (x1 + (xVel * interpolation)), (int) (y1 + pipeVerticalSpacing), pipeWidth, (int) Menu.HEIGHT);

		// 2
		g.fillRect((int) (x2 + (xVel * interpolation)), 0, pipeWidth, (int) y2);
		g.fillRect((int) (x2 + (xVel * interpolation)), (int) (y2 + pipeVerticalSpacing), pipeWidth, (int) Menu.HEIGHT);

		// 3
		g.fillRect((int) (x3 + (xVel * interpolation)), 0, pipeWidth, (int) y3);
		g.fillRect((int) (x3 + (xVel * interpolation)), (int) (y3 + pipeVerticalSpacing), pipeWidth, (int) Menu.HEIGHT);
